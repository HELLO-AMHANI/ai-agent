...
PRICE_MONTHLY  = "per month · cancel anytime"
...
.acct-email { color: var(--muted); letter-spacing: 0.03em; }
.acct-badge {
    font-size: 0.58rem;
...
.udot.last   { background: var(--red); border-color: var(--red); }

/* ═══════════════════════════════════
   PLAN BADGE (subscribers)
═══════════════════════════════════ */
.plan-badge {
    display: flex;
    align-items: center;
...
.paywall-icon { font-size: 1.5rem; margin-bottom: 0.8rem; color: var(--gold); opacity: 0.6; }
.paywall-title {
    font-family: 'Cormorant Garamond', serif;
    font-size: 1.9rem;
    font-weight: 600;
...
if not subscribed:
    ...
    v_limited   = is_limited(VISITOR_ID)

    dots = ""
    for i in range(FREE_LIMIT):
        if i < v_used:
            css = "udot last" if i == FREE_LIMIT - 1 and v_remaining == 0 else "udot filled"
        else:
            css = "udot"
        dots += f'<div class="{css}"></div>'

    label_style = "color:#E74C3C;" if v_remaining <= 1 else ""
    st.markdown(f"""
    <div class="usage-bar">
        <span style="{label_style}">Free consultations left: <strong>{v_remaining}</strong></span>
    ...
    if v_remaining == 1:
        st.markdown("""
        <div class="warn-bar">⚠ &nbsp; Last free consultation — subscribe to continue after this.</div>
        """, unsafe_allow_html=True)
else:
    st.markdown("""
...
if not subscribed and is_limited(VISITOR_ID):
    ...
    <div class="paywall">
        ...
        <div class="paywall-body">
            ...
            Subscribe to CONSULTAMHANi for unlimited AI-powered financial intelligence — available 24/7.
        </div>
        <div class="paywall-stats">
            <div>
                <span class="pstat-num">{FREE_LIMIT}</span>
                <span class="pstat-label">Used</span>
            </div>
            <div>
                <span class="pstat-num">∞</span>
                <span class="pstat-label">With Pro</span>
            </div>
            <div>
                <span class="pstat-num">24/7</span>
                <span class="pstat-label">Always On</span>
            </div>
        </div>
...
if not st.session_state.messages:
    greeting = "Welcome back to CONSULTAMHANi" if subscribed else "What would you like to explore?"
    st.markdown(f"""
    <div class="welcome-wrap">
        <div class="welcome-title">{greeting}</div>
        <div>
            <span class="welcome-example">"Get stock price for TSLA"</span>
            <span class="welcome-example">"P/E ratio: price $200, EPS $10"</span>
            <span class="welcome-example">"Latest Apple earnings news"</span>
            <span class="welcome-example">"Compare AAPL vs MSFT"</span>
            <span class="welcome-example">"What is market capitalisation?"</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
